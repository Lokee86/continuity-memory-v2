...
Direct `openai-ready` embedding and General-model transport are implemented. `vectors probe` tests the configured live embedding endpoint and `vectors build` establishes/reuses its compatibility profile and publishes a real Archive vector generation. Insomnia can use its own General-model route or fall back to `models.general`.
