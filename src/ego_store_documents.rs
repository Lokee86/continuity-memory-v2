...
use crate::{Container, EgoError, EgoIdentity, EgoPersonality, EgoWebSynthesis};
...
impl EgoStore {
    pub(crate) fn put_identity(
        ...
        let actual_revision = self.identity.as_ref().map_or(0, |value| value.revision);
        check_revision(expected_revision, actual_revision)?;
        if self
            .identity
            .as_ref()
            .is_some_and(|value| value.text == text)
        {
            return Ok((self.identity.clone().unwrap(), false));
        }
        let revision = next_revision(actual_revision)?;
        let record = EgoRecord::Identity {
            ego_version: self.next_version()?,
            revision,
            text: text.clone(),
        };
        self.append_apply(container, record)?;
        Ok((EgoIdentity { revision, text }, true))
    }
