use crate::{
    ArchiveVectorId, ChunkRef, EmbeddingProfileId, VectorGenerationError, VectorGenerationId,
};

const FORMAT_MAGIC: &[u8; 8] = b"CVAVGFM1";
const FORMAT_SCHEMA: u32 = 1;
const GENERATION_MAGIC: &[u8; 8] = b"CVAVGEN1";
const VERSION_MAGIC: &[u8; 8] = b"CVAVGRC1";
...
pub(crate) fn encode_generation(payload: GenerationPayload) -> [u8; 112] {
    ...
    out[40..72].copy_from_slice(&payload.profile_id.0);
...
pub(crate) fn decode_generation(
    ...
    Ok(Some(GenerationPayload {
        ...
        profile_id: EmbeddingProfileId(bytes[40..72].try_into().expect("profile id")),
