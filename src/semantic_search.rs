...
impl Cva {
    ...
    pub fn semantic_search_f64(
        ...
        let vectors = endpoint.embed_f64(EmbeddingMode::Query, &[query.to_string()])?;
        let query_vector = vectors
            .first()
            .filter(|v| v.len() == profile.dimensions as usize && v.iter().all(|x| x.is_finite()))
            .ok_or(SemanticSearchError::InvalidQueryVector)?;
        let generation = self
            .vector_generations
            .current_generation(profile_id)
        ...
        if schema.scalar != ScalarType::F64 {
            return Err(SemanticSearchError::UnsupportedScalar(schema.scalar));
        }
        if usize::try_from(schema.dimensions).ok() != Some(query_vector.len()) {
        ...
        let qnorm = vector_norm_f64(query_vector).ok_or(SemanticSearchError::InvalidQueryVector)?;
        ...
        for ordinal in 0..packed.count() {
            ...
            if let Some(score) = cosine_f64_row(row, query_vector, qnorm)?
                && score > 0.0
            {
...
fn vector_norm_f64(vector: &[f64]) -> Option<f64> {
    let mut norm = 0.0_f64;
    for value in vector {
        if !value.is_finite() {
            return None;
    ...
    (norm > 0.0 && norm.is_finite()).then(|| norm.sqrt())
}
...
fn cosine_f64_row(
    ...
    if row.len() != query.len().saturating_mul(8) {
        return Err(SemanticSearchError::CorruptMatrix("f64 row width"));
    }
    let mut dot = 0.0_f64;
    let mut row_norm_squared = 0.0_f64;
    for (encoded, query_value) in row.chunks_exact(8).zip(query) {
        let value = f64::from_le_bytes(encoded.try_into().expect("f64 width"));
        if !value.is_finite() {
    ...
    if row_norm_squared == 0.0 || !row_norm_squared.is_finite() {
        return Ok(None);
    }
