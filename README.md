...
- single-pass reopen reconstruction that feeds Archive, packed-vector, and Archive-Vector rebuild from the same physical scan while Container validates framing/global tickets;
...
The current development format requires Archive marker `CVAAFMT2`, packed-vector marker `CVAPVFM1`, and Archive-Vector marker `CVAAVFM1`. Earlier development CVAs are rejected; migration code is intentionally not implemented yet.
...
- embedding profiles, semantic vector generations, or exact vector search;
- Memories, Graph, or Memory Vector databases;
- shared Continuity runtime, retrieval ranking, Insomnia, Dream, or Ego.

The current indexes remain derived acceleration state; they do not alter Archive, packed-vector, or Archive-Vector authority. Reopen performs one streaming physical chunk pass. Current-format measurements are refreshed in `docs/development.md`; a cold-cache sample and larger vector-bearing measurements remain required.
