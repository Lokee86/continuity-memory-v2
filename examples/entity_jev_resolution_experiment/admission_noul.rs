...
fn run_case(
    ...
    if !remaining.is_empty() {
        ...
        for (index, _) in remaining.iter().enumerate() {
            questions.insert(format!("m{index}"), json!({
                "type":"noul",
                "instructions":format!(
                    "Does mention index {index} denote a durable reusable identity that should exist as an Entity beyond this sentence or Memory?"
                ),
                "criteria":{
                    "true":"A stable named, owner-relative, artifact, component, system, person, organization, place, or other reusable identity. First-seen identities count as true.",
                    "false":"A generic/context-only role, abstract process/state/action, transient runtime value/role, sentence-local object, or wrapper/category/activity phrase that should not become a durable Entity."
                }
            }));
        }
        let state = json!({
        ...
        match client.evaluate(state, Value::Object(questions)) {
            Ok(response) => {
                ...
                for (index, mention) in remaining.iter().enumerate() {
                    match noul(&response, &format!("m{index}")) {
                        Ok(probability) => {
                            let decision = if probability >= 0.5 {
                                "create_new"
                            } else {
                                "reject"
                            };
                            model.push(json!({"candidate":mention,"decision":decision,
                                "noul":probability,"probabilities":{"durable_entity":probability}}));
                        }
                        Err(e) => {
                            error = json!(e);
    ...
    json!({"memory_id":id,"owner_id":owner,"expected_entity_mentions":prior["expected_entity_mentions"],
...
fn fallback(mention: &Value) -> Value {
    json!({"candidate":mention,"decision":"unresolved","noul":null,"probabilities":null})
}
