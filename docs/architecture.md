...
### Model switchboard
`ModelSwitchboardConfig` owns machine-local endpoint selection for explicit model capabilities. The current capabilities are `General`, `Insomnia`, `InsomniaMetadata`, `Dream`, and `Embedding`; the providers are `OpenAiCodex` and `OpenAiReady`. Main Insomnia and Dream may each have dedicated routes and otherwise resolve to General. Insomnia metadata is optional and does not inherit General; when owner routing is enabled, ownership classification prefers the metadata route and otherwise falls back to effective main Insomnia. Each configured route carries a stable `CredentialId`. `ModelCapability::route_key()` is the canonical external capability-key vocabulary used by host adapters.
...
The same `InteractionRuntime` also exposes the bounded Project-REL semantic coordination seam through `process_background_work`: it drains Insomnia, fills missing Memory vectors, derives Dream backlog from active `extracted` Memories, and runs bounded Dream work against its owned `Cva`. The long-lived `ReliquaryRuntimeHost` now performs the corresponding continuous coordination for both its active REL and optional attached PHY. Each owner becomes Dream-eligible only after its own compatible Memory-vector binding exists. Dream owns no second scheduler, durable queue, Container handle, service, or process. Host inference snapshots one owner under its lock, releases all REL/PHY/runtime locks for classifier/verifier calls, then revalidates the target Graph version and all participating Memory revisions before owner-local publication. Successful lifecycle projection removes work from the derived backlog; classifier/verifier failure leaves the source `extracted` for bounded retry.
...
Each snapshot carries a derived `generation`, its exact `derived_graph_version`, the algorithm version/configuration, quality value, and a complete partition of the current Graph nodes. Community generation is derived-state bookkeeping only and consumes no semantic/global version ticket. A snapshot becomes stale as soon as `graph_version` changes. Refreshing an unchanged current v2 snapshot is an idempotent no-op; refreshing stale or older-algorithm state performs a complete scan-and-merge pass and appends the next derived generation. Supported older derived snapshots remain readable so an algorithm upgrade cannot make an owner file unopenable.

`CommunityId` is scoped by the durable owner UUID and exact sorted membership, so equal membership in the same owner is stable while changed membership deliberately produces a different ID. Stable identity across split/merge changes and lineage are not implied by scan-and-merge. REL and PHY snapshots are strictly owner-local. Human-readable names are presentation metadata for Warlock or another host and do not participate in membership, Graph semantics, or identity. A real semantic reconciliation repack may omit community snapshots and rebuild them from the merged Graph; an already-absorbed no-op reconciliation preserves canonical bytes and therefore preserves existing snapshots. See [ADR 0025](decisions/0025-owner-local-derived-communities.md).

A private test-only routing experiment now derives transient `CommunityRoutingProfile` values from one persisted Community snapshot plus existing Memory vectors. Profiles contain a `CommunityId` and deterministic existing `MemoryRef` representatives selected by medoid, diverse-vector, or structural-central strategies; no new Memory/vector/Graph semantic object is persisted. The corrected fixture follows the normal legacy-CVA → typed Project REL migration path, runs Dream to completion, persists current Community algorithm v2 over the resulting Graph, and only then benchmarks routing. On the 46-Memory real Dream Graph (155 active relations, seven communities), five routed communities recover up to 97.10% of the global 12-neighbour semantic baseline but still admit about 77% of all Memory vectors. This seam therefore remains disconnected from production retrieval and traversal; see [Community routing benchmark — 2026-08-28](community-routing-benchmark-2026-08-28.md).
...
The source and every returned candidate are materialized as `DreamMemoryContext`: current Memory metadata/body identity, authoritative source timestamp when one is available, derived temporal analysis, and all active Graph relationships touching that Memory. Current Memories use persisted `source_time_ns` first. Legacy records without it resolve adopted assistant content provenance first, then the exact user-authority turn through Episode provenance, with the Episode source boundary only as a fallback. `Memory.created_at_ns` and grounding-only provenance do not define semantic source time.
