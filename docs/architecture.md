...
CommunityStore owns persisted **derived organization over one owner's Graph**, not relationship authority. `Cva::refresh_communities_leiden` and `Phylactery::refresh_communities_leiden` materialize a full-Graph Leiden partition for the current REL or PHY. The baseline projects active oriented Graph relationships to undirected structural pairs, collapses multiple relationship identities between the same Memory pair to one unweighted structural edge, and runs modularity optimization at resolution `1.0` with a fixed seed and deterministic sequential execution. Graph records are never added, removed, redirected, or rewritten by clustering.

Each snapshot carries a derived `generation`, its exact `derived_graph_version`, the algorithm version/configuration, quality value, and a complete partition of the current Graph nodes. Community generation is derived-state bookkeeping only and consumes no semantic/global version ticket. A snapshot becomes stale as soon as `graph_version` changes. Refreshing an unchanged current snapshot is an idempotent no-op; refreshing stale state reclusters the complete owner Graph and appends the next derived generation.

Baseline `CommunityId` is scoped by the durable owner UUID and exact sorted membership, so equal membership in the same owner is stable while changed membership deliberately produces a different ID. Stable identity across split/merge changes, lineage, and affected-region scan-and-merge are not implied by the baseline. REL and PHY snapshots are strictly owner-local. Human-readable names are presentation metadata for Warlock or another host and do not participate in membership, Graph semantics, or identity. A real semantic reconciliation repack may omit community snapshots and rebuild them from the merged Graph; an already-absorbed no-op reconciliation preserves canonical bytes and therefore preserves existing snapshots. See [ADR 0025](decisions/0025-owner-local-derived-communities.md).
