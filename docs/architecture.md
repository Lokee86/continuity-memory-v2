...
PHY principal identity and presentation metadata are deliberately separate. The header UUID derives the stable `phy-<uuid>` principal identity. `PhylacteryProfile` stores optional mutable `display_name` and `username` values as latest-wins owner metadata. Ego `Identity.name` remains the label of a selectable persona/identity document, not the human principal's durable name. When a REL has materialized the matching reserved principal Entity, RuntimeHost synchronizes current profile values only into that Entity's aliases/summary; canonical name and EntityId remain derived from `phy-UUID`, and principal aliases remain excluded from ordinary Entity candidate indexing.
