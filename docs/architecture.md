...
Reliquary and Phylactery use the same Graph durability/versioning inside their own files. The pinned `arcana` crate supplies repository-agnostic graph primitives, adjacency/topology structures, and traversal algorithms through its reusable library boundary. Reliquary owns typed endpoint identity, relation vocabulary, publication/versioning, and producer-specific validation. Dream remains the semantic producer for Memory-to-Memory relation families; Perception will publish Entity/Observation relation families. Pre-Graph legacy CVAs reopen with an empty Graph owner and acquire the Graph format marker lazily on the first relationship write. The Graph remains owner-local: typed node identity does not create cross-file edges. See [ADR 0036](decisions/0036-typed-semantic-graph-endpoints.md).
