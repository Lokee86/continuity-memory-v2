...
1. Run a controlled `v2-6` model-quality comparison using the tracked 40-case gold set: three fresh Luna-low runs versus three runs at the next reasoning level, scoring selection/provenance/metadata/receipt guards mechanically and judging semantic-target agreement explicitly. Use that result to decide whether the remaining selection instability is primarily reasoning-depth limited. A focused 32/48/64 worker confirmation remains required after semantic/model tuning settles.
2. If unsupported semantic elaboration remains after the reasoning comparison, design a semantic-support validation/review mechanism against the gold failures rather than continuing to expand lexical heuristics. General semantic entailment is not currently validated deterministically.
