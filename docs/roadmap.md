...
1. Design the next Insomnia semantic-support validation/review layer against the tracked gold failures rather than continuing to expand lexical heuristics. The low-versus-medium reasoning comparison did not improve semantic correctness, so the next work should target unsupported source selection, question-to-decision conversion, deictic provenance, incomplete semantic targets, and non-numbered execution-state leakage directly. General semantic entailment is not currently validated deterministically.
2. Re-run the tracked gold comparison after that semantic-support change, then perform the focused 32/48/64 worker confirmation once semantic/model behavior has settled. Keep Luna-low as the current reasoning default unless a later measured comparison shows a material quality gain.
