...
35. Gold v3 now separates exact anchor fidelity from explicitly allowed semantic-state source coverage and corrects superseded/alternate-authority cases. Three complete Sol-low v2-7 controls averaged `81.2%` state coverage. An opt-in two-pass `v2-8-ledger` experiment using Sol-high for both authority disposition and synthesis completed 47/66 Episodes before the Plus Codex usage limit was exhausted; among the 29 gold-v3 cases whose Episodes completed, state coverage reached `95.8%` while omit cleanliness fell to `60.0%`, with substantial over-atomization. The unmeasured `v2-9-ledger` revision tightens transient/question/supersession/adoption policy and adds exact-cover consolidation. It remains experimental until a complete live rerun.
36. Atomic self-cleaning Insomnia completion: Memory bodies/global-version tickets may be staged inertly, but newly created Memory records and the final Episode outcome become visible only through one `CVAINSC1` completion chunk. Reopen truncates an incomplete trailing chunk, so interruption before/during completion yields no partial Memory publication and the claim is reclaimable. Successful completion collapses prior attempt history to one compact receipt; retry/terminal failures persist only current Work state rather than an append-only attempt log.
...
1. Complete live validation of experimental **v2-9-ledger** on the small routine tuning corpus first: `corpus/insomnia-tuning-v1.jsonl` contains 11 isolated Episodes / 314 turns with 19 matching gold-v3 cases. Use Sol-high for both passes while validating the decomposition, and require that the v2-8 state-coverage gain survives while phase-renumbering/Godot-first precision, short adoption, supersession, governing-plan retention, grounding discipline, and Memory count improve. Reserve the full 66-Episode fixture for one milestone confirmation after the small corpus is stable. Do not promote the ledger path from opt-in until that confirmation and strict semantic-target audit pass.
2. After the two-pass architecture is validated, tune model/reasoning cost and throughput independently on the 11-Episode fixture, then perform one full gold-v3 confirmation and only afterward recalibrate worker concurrency for the chosen model mix. Do not reuse the old 48-worker Luna-low optimum for Sol-high or other heavier models. Keep the one-pass v2-7 path as the comparison baseline until replacement is justified; do not add vector retrieval or another blanket semantic reviewer unless a measured case demonstrates a distinct need.
