...
1. Complete live validation of the experimental **two-pass authority/disposition ledger** against gold v3. The current unmeasured `v2-9-ledger` contract must run the full 66-Episode fixture with Sol-high for both passes and demonstrate that the v2-8 state-coverage gain survives while phase-renumbering/Godot-first precision, short adoption, supersession, governing-plan retention, grounding discipline, and Memory count improve. Do not promote the ledger path from opt-in until a complete run and strict semantic-target audit pass.
2. After the two-pass architecture is validated, tune cost and throughput independently: test lower reasoning/smaller models for disposition and synthesis, then recalibrate worker concurrency for the chosen model mix. Do not reuse the old 48-worker Luna-low optimum for Sol-high. Keep the one-pass v2-7 path as the comparison baseline until replacement is justified; do not add vector retrieval or another blanket semantic reviewer unless a measured case demonstrates a distinct need.
