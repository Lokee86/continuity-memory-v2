...
1. Recheck 32/48/64-worker processing under restored extractor contract `v2-3`, then inspect fresh `v2-3` whole-file outputs for extraction quality, noise, run-to-run Memory variance, rejected candidates, and historical-evidence behavior. The prior concurrency corpus used shortened contract `v2-2`, so its Memory-count variance is not a valid quality result for the restored legacy policy; targeted callback cases still need explicit quality inspection.
