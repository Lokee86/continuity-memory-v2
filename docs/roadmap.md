...
1. Build the next semantic-support/review seam on top of the completed v2-7 provenance split. Evaluate whether each proposed Memory is entailed by the authoritative user proposition plus only the permitted role of its authority/grounding sources. Exhaust deterministic current-Episode/nearby ancestry before any broader retrieval; vector retrieval remains a fallback only for genuinely unresolved distant references.
2. Run fresh gold-v2 Luna-low comparisons after the semantic-support change, then perform the focused 32/48/64 worker confirmation once semantic/model behavior has settled. Keep Luna-low as the current reasoning default unless a later measured comparison shows a material quality gain.
