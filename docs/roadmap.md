...
1. Rerun the five-run Insomnia quality audit under `v2-5` and compare durable-source recall, false-memory rate, provenance correctness, receipt leakage, metadata stability, and cross-run source selection before further model tuning. A focused 32/48/64 worker confirmation remains required after semantic hardening.
