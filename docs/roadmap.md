...
1. Split Insomnia semantic support into distinct roles before adding broader retrieval: **user authority**, **adopted assistant content**, and **referent/context grounding**. The 40-case account-context audit found that all 9 gold cases requiring contextual provenance already have enough nearby conversation context, while account-wide related state can over-expand or distort the current proposition. Exhaust deterministic current-Episode/nearby ancestry first; use vector retrieval only as a fallback for genuinely unresolved distant references. The current `content_source` model conflates adoption with referent grounding and must be corrected.
2. Re-evaluate/correct the gold cases under that provenance split, especially ShipStats (`ok, so we do want to add it now` reverses the assistant's recommendation and only uses assistant context to resolve `it`). Then implement the semantic-support/review seam and rerun the tracked gold comparison before the focused 32/48/64 worker confirmation. Keep Luna-low as the current reasoning default unless a later measured comparison shows a material quality gain.
