...
### Insomnia two-pass authority ledger experiment — 2026-08-16

The next experiment split the v2-7 one-pass `Episode -> Memories` task into two independent Sol-high calls. Pass 1 produces a complete authority/disposition ledger that accounts for every user turn and separates retainable propositions from questions, receipts, transient execution, adoption, correction, and referent grounding. Pass 2 receives only retained ledger units and synthesizes standalone Memories; it cannot create authority that pass 1 did not retain. The experimental path is opt-in through the repo-local CLI and records its own contract version in durable Insomnia attempt history, leaving the v2-7 path intact for comparison.

The first live contract, `v2-8-ledger`, was run over a fresh copy of the 66-Episode fixture using `gpt-5.6-sol` with high reasoning for both passes. A 48-worker attempt was abandoned as an operationally poor shape for Sol-high. A 16-worker run completed 47 Episodes before the ChatGPT Plus Codex usage bucket was exhausted; the remaining 19 Episodes terminaled with provider `usage_limit_reached` errors rather than semantic/runtime failures. The 47 completed Episodes created 358 Memories and rejected 26 candidates. Because the provider cap prevented 19 Episodes from running, whole-gold percentages from that file are not comparable to complete controls.

Restricting gold v3 to the 29 cases whose Episodes actually completed gives `86.2%` exact anchor fidelity, `95.8%` durable-state source coverage (`23/24`), and `60.0%` omit cleanliness (`3/5`). The source-selection improvement is the important positive signal: the same gold-v3 metric averaged `81.2%` state coverage across the three complete Sol-low one-pass controls. The two-pass run recovered the narrow server-authoritative collision proposition and correctly omitted the Prompt-41 execution receipt. The one processed durable-state miss was the short `alright, that works` adoption of the server-response change.

`v2-8-ledger` was not good enough to promote. It retained transient phase renumbering and the Godot-first plan-conflict question, retained the older player-color predecessor even though later same-Episode authority superseded it, and selected the packet-migration source without preserving the governing migration architecture. It also over-atomized durable turns: 358 Memories came from 185 unique source nodes, with 61 sources producing multiple Memories and one source producing 14. Grouping the observed output only by source/provenance/type/category would still leave about 265 groups, so synthesis alone cannot fix this; the disposition pass also needs to stop promoting temporary implementation mechanics.

The revised unmeasured contract is `v2-9-ledger`. Pass 1 now explicitly treats phase/prompt numbering and execution scaffolding as transient, rejects recalled/questioned plans as renewed authority, preserves short confirmations such as `alright, that works` as adoption when the preceding assistant proposition is concrete, requires long authoritative requirement blocks to survive even when a trailing request is also present, distinguishes pasted agent material from explicit user endorsement, suppresses superseded predecessors, avoids unnecessary grounding for self-contained user assertions, and instructs the model not to atomize one coherent retained design into one unit per implementation detail. Pass 2 now uses exact-cover synthesis: every retained ledger id must be covered exactly once, but compatible units may be consolidated into a smaller Memory. Consolidation is limited to the same source node, authority/provenance, semantic type, and lifecycle/modality; the final category must already exist among the covered units. Deterministic validation rejects dropped ids, duplicate coverage, unknown ids, and incompatible merges.
...
Operational retention is deliberately narrow. Pending queue state, claims, leases, renewals, retry counters, retry delays, and retryable failures now exist only in memory and emit no `CVAINSW1` chunks. On reopen, every Episode without a final success or terminal outcome is derived as Pending with a fresh runtime attempt count. Successful processing retains one compact `CVAINSC1` receipt containing the producing model/contract, timestamps, Memory IDs, and rejected count. Terminal outcomes still use one durable final `CVAINSW1` record during this compatibility slice. Detailed extraction responses, evidence bundles, disposition/synthesis intermediates, and retry traces are not made durable.

This removes normal Insomnia Work churn at the source rather than depending on vacuum: queue/claim/renew/retry transitions add zero CVA bytes. Older development CVAs can still contain superseded `CVAINSW1` records, which remain readable but are treated as compatibility history; transient old states are ignored when the scheduler is rebuilt. The remaining crash-garbage seam is staged Memory bodies/global-version tickets written before `CVAINSC1`; making the completion transaction self-contained is the next storage-format step.
