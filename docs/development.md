...
src/embedding_profile*.rs           profile probing/identity/reopen
...
The vector smoke copies the source CVA, creates two deterministic simulated endpoints with identical advertised model metadata but different behavior seeds, creates two profiles, builds one full Archive generation per profile, syncs, reopens, and verifies both current generations.
...
median:          27.806 ms
p90:             29.498 ms
retained heap:   757,016 bytes
peak additional: 888,695 bytes
```
...
The same corpus with two simulated profiles and two current generations is `2,288,440` bytes. Each profile uses 32-dimensional `f32` rows and covers all 281 fragments, for `562` total Archive-Vector rows.
...
median:          27.401 ms
p90:             28.725 ms
retained heap:   759,634 bytes
peak additional: 909,469 bytes
```
