...
Before that structural experiment, gold/evaluator v3 should correct the stale player-color implementation target and distinguish exact-anchor fidelity from semantic coverage by valid alternate/later authority. No result in this audit supports adding vector retrieval; vectors cannot fix propositions that are already present in the Episode but misclassified or skipped.
