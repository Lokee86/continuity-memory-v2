...
More importantly, all 9 gold cases currently marked as requiring contextual provenance already have enough nearby conversation context to resolve the needed proposition or referent. Seven are resolved by the immediately preceding assistant turn; the other two are resolved within a few turns. The current gold set therefore does not demonstrate a need for vector search as the first semantic-support mechanism. Deterministic nearby ancestry/current-Episode context should be exhausted first; vector retrieval is better treated as a fallback for genuinely distant unresolved references.

The audit also exposes a provenance-modeling bug. `content_source` currently conflates **adopted assistant content** with **referent grounding**. `adopt-shipstats-now` is the clearest example: the preceding assistant recommends *not* adding ShipStats yet, and the user replies `ok, so we do want to add it now`, reversing that recommendation. The assistant context is needed to resolve `it` as ShipStats, but it is not adopted authority for the later thin/deferred scope. `deleted-assets-contextual-correction` has the same structural distinction: nearby assistant context identifies what `them` refers to, while the user turn itself authorizes the deletion correction. A future semantic-support layer must represent authority provenance separately from referent/context grounding.
