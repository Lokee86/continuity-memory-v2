...
The frozen v2f candidate uses semantic workstream/scope as the boundary, adds explicit bounded-session/lifecycle handling, and tightens verbatim evidence-copy instructions. Three independent live `gpt-5.6-luna` low-reasoning runs scored `37/40`, `39/40`, and `38/40`; related recall was `27/30`, `29/30`, and `28/30`, unrelated accuracy was `10/10` in every run, and all three had zero output errors. The sole persistent miss was one medium-confidence `ship_runtime_identity` judgment; remaining misses moved between runs. The candidate therefore clears the exact-pair advancement gate three times consecutively and is frozen in `corpus/dream-classifier-v2-candidate.txt`. Production remains on classifier v1 pending the required 46-Memory population confirmation documented in [Dream classifier v2 candidate design](dream-classifier-v2-design.md).
