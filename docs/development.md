...
Successful Episode processing no longer publishes local REL Memory revisions or their backing objects one at a time. The processor validates extraction and prepares new content-addressed local Memory bodies, Memory records, and contiguous REL-global version assignments entirely in memory, then writes one current `CVAINSC3` completion transaction containing the new local bodies, every newly visible local Memory record, the owned global-version range, owner-qualified external Memory references, and the compact successful Episode receipt. The same chunk is consumed by both REL Memory and Insomnia rebuild state. `CVAINSC1` and `CVAINSC2` completions remain readable for development compatibility; V1/V2 reopen with no external references.

Container reopen treats an incomplete final length-prefixed chunk as an interrupted append: it truncates the file back to the start of that trailing chunk and continues from the last complete boundary. The existing deterministic truncation suite continues to cover the atomic local Memory transaction through the version-compatible completion decoder, while current writes are `CVAINSC3`. Routed ownership adds a second crash boundary: a PHY User Memory is synced before the REL receipt, and a dedicated retry regression verifies that deterministic mutation identity reuses that already-durable PHY Memory rather than duplicating or rewriting it. A separate regression runs 1,000 synthetic processing failures before success and verifies exactly zero REL growth before success and exactly one new completion chunk.

Operational retention is deliberately narrow. Pending queue state, claims, leases, renewals, retry counters, retry delays, and retryable failures exist only in memory and emit no `CVAINSW1` chunks. On reopen, every Episode without a final success or terminal outcome is derived as Pending with a fresh runtime attempt count. Successful processing retains one compact `CVAINSC3` REL transaction containing the producing model/contract, timestamps, local Memory IDs, owner-qualified external Memory references, rejected count, newly required local Memory bodies/records, and their REL-global version range. Terminal outcomes still use one durable final `CVAINSW1` record during this compatibility slice. Detailed extraction responses, evidence bundles, classification/synthesis intermediates, and retry traces are not made durable.
...
The hard selector ranking from this 2026-08-24 tuning closure remains `Sol-low > Ox Alpha-low >> Luna-low`. Since that measurement, authoritative extractor contract `v3-1` has ported the fixed-group metadata seam and added the separate bounded `user | project` durable-owner classifier before wording. Those later routing changes do not revise the historical model-quality measurements above. Further semantic reliability work should require a materially different capability boundary such as stronger/fine-tuned models, voting/verifier architecture, deterministic semantic preprocessing, or another separately justified structural stage.
