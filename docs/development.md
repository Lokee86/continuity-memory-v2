...
Extractor contract `v2-4` therefore adds a required structured `authority_kind` field with `direct`, `correction`, `adoption`, and `retention` values. The prompt now explicitly forbids turning a merely interrogative implication into an asserted memory and requires vague/deictic adoption or retention turns to identify assistant-authored content through the content-source fields. This slice makes authority explicit in structured output only; stronger deterministic semantic/provenance rejection remains the next hardening step.
