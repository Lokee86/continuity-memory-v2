...
`corpus/dream-classifier-regressions-v1.json` now freezes the discovered decision boundary as 40 classifier cases: 30 related and 10 unrelated. The positive side groups same-workstream relations such as packet policy/implementation, multiplayer-flow state, workspace operations, ship runtime/identity, asteroid lifecycle, network protocol, status summary/detail, conceptual analogy, and general-to-scoped interaction preferences. The negative side groups cross-project history leakage, cross-subsystem same-project links, and lexical collisions across different entities. Seven weaker positive judgments are marked medium-confidence so future prompt work does not silently overfit them. `tools/build_dream_classifier_regressions.py` reconstructs the fixture from the durable v1/v2 reports and tracked web audit; `tools/evaluate_dream_classifier_regressions.py` scores future reports by expected relatedness, confidence, and pattern. On this boundary fixture, classifier v1 scores 29/30 related but 0/10 unrelated (29/40 overall); the rejected v2 scores 1/30 related and 10/10 unrelated (11/40 overall). The next prompt must preserve same-workstream recall while blocking only cross-workstream/project-context leakage.
