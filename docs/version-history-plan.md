...
Not implemented: generic full historical `ArchiveView`, multi-database whole-CVA restore activation, retention/vacuum, and concurrent multi-writer publication.
...
1. Compact Archive lookup representation keyed to an Archive watermark.
2. Optional Archive checkpoint load + replay tail.
3. Archive Vectors as a second concrete store with its own local sequencing if needed.
4. Prove global ordering across both stores without semantic coupling.
5. Define read-only whole-CVA point materialization.
6. Only then define crash-safe whole-CVA restore-and-continue timeline semantics.
7. Define retention/vacuum reachability for old local branches/timelines.
...
- Whether Archive Vectors need a local counter identical to Archive's.
