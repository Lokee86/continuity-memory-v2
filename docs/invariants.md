...
82. **Graph topology remains same-owner even with typed semantic nodes.** REL and PHY may each catalogue Memory, Entity, and Observation node identities, but typed `SemanticNodeRef` values are owner-local and do not establish cross-file Graph relationships. Current durable relation mutation records remain Memory-to-Memory until typed relation persistence is implemented under ADR 0036.
