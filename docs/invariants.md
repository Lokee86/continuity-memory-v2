...
82. **Graph topology remains same-owner with typed semantic nodes and relations.** REL and PHY may each catalogue Memory, Entity, and Observation node identities, but typed `SemanticNodeRef` values are owner-local and do not establish cross-file Graph relationships. Accepted typed relation families validate endpoint kinds and owner-local object existence; `Memory -> Entity` `entity-association` is the first Perception-owned family under ADR 0036.
