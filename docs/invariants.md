...
85. **User-global export is conservative and source-independent.** Without an ownership classifier, Insomnia defaults to the active REL. A User-owned result may enter PHY only through an explicit routing target, and REL-local provenance is removed before PHY publication.
