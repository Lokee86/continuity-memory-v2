...
1. **One file, several databases.** A Reliquary `.rel` contains explicit purpose-built databases, not one generalized semantic database; legacy `.cva` and typed REL forms are compatibility inputs, not current REL classes.
...
27. **Shared scan does not imply shared semantics.** Archive, Memories, Entities, Graph, packed-vector, Archive-Vector, profile, and generation rebuild logic consume the same physical payload stream but classify and validate their own records explicitly.
...
60. **Every current durable owner is born identified.** New REL/PHY creation persists one UUID in the container header before returning.
61. **Owner IDs identify file ownership, not organizational taxonomy.** Current durable IDs are `rel-<uuid>` or `phy-<uuid>`; REL type labels, filenames, and optional adapters are not identity. Legacy typed REL prefixes remain compatibility identities only.
...
66. **Graph owns typed semantic relation-edge authority; capital-R Relationships are a separate Perception owner.** Graph relation state is not Memory/Entity/Observation metadata and does not belong to Archive, Dream/Perception runtime state, or the physical Container. Dream remains the semantic producer for Memory-to-Memory Graph relation families; Perception owns Graph relation families involving Entity/Observation nodes. ADR 0034 Relationship objects are separately persisted owner-local semantic containers over owner-qualified Entity references and are not Arcana/Graph edges.
67. **Graph node identity is typed and owner-local.** Stable `SemanticNodeRef` values distinguish Memory, Entity, and Observation identities before mapping to dense internal `NodeId` values. A bare semantic-node reference is not durable cross-owner identity, and the Graph does not persist cross-owner endpoints.
...
71. **Generic graph mechanics do not own Reliquary semantics.** The reusable `arcana` library may supply topology, storage mechanics, and traversal algorithms; Reliquary owns typed semantic endpoints, relationship vocabulary, REL/PHY publication/versioning, and producer-specific Dream/Perception semantics.
...
73. **Divergent Graph reconciliation follows owner dependencies.** Memories and Entities are replayed before Graph endpoints, relationship authority is republished through Graph with fresh clocks, and topology/derived indexes rebuild from merged authority instead of becoming synchronization metadata.
...
78. **Reliquary identity is stored; organizational type is metadata.** New REL files carry authoritative Reliquary file kind plus durable UUID and no behavioral scope discriminator. Optional type labels may organize UI only. Legacy typed headers remain readable and opening them must not silently rewrite their physical identity.
79. **Phylactery identity is distinct and stored.** New PHY files carry authoritative Phylactery file kind and durable UUID; REL and PHY lifecycles must reject one another when their file identities do not match.
80. **Phylactery does not depend on source retention.** A current PHY Memory is valid without source turns, Episodes, Files, or a live originating REL. Direct PHY publication rejects REL-local provenance fields; routed Memories may retain an identifier-only owner-qualified `MemorySourceRef` whose target can remain unresolved when the source REL is unavailable.
81. **Shared codecs do not collapse file semantics.** Phylactery currently persists Memories, Entities, Relationships, Graph, Packed Vectors, Memory Vectors, Compatibility Profiles, and PHY-owned Ego state; REL-only owners cannot appear in PHY merely because the shared container can physically carry their bytes.
82. **Graph topology remains same-owner with typed semantic nodes and relations.** REL and PHY may each catalogue Memory, Entity, and Observation node identities, but typed `SemanticNodeRef` values are owner-local and do not establish cross-file Graph relationships. Accepted typed relation families validate endpoint kinds and owner-local object existence. Ordinary `Memory -> Entity` `entity-association` is Perception-owned; REL-only PHY-backed principal identity uses the reserved `principal` Entity class plus `Memory -> Entity` `principal-association`, which is durable but excluded from ordinary semantic traversal, candidate generation, reconciliation, Knowledge visualization, Memory projection, and Leiden. Principal traversal is explicit-only, and PHY rejects self-principal Entity materialization.
...
84. **Insomnia destination ownership cannot redefine semantic identity.** User/active-REL routing is decided only after semantic groups are fixed; ownership cannot change propositions, authority, provenance, metadata/lifecycle, grouping, or candidate key material.
85. **User-global export is conservative and reference-preserving.** Without an ownership classifier, Insomnia defaults to the active REL. A User-owned result may enter PHY only through an explicit routing target; REL-local provenance fields are removed before PHY publication, but their durable owner/source identities are retained in `MemorySourceRef` without copying source records.
...
87. **External provenance identity and chronology are separate.** Insomnia persists identifier-only external provenance as `Memory.source_ref` and source-derived semantic chronology as `Memory.source_time_ns`; neither requires copying the originating source records, and chronology must never be replaced with Memory creation/update bookkeeping.
...
89. **Unknown semantic chronology stays unknown.** REL Dream prefers validated Archive/Episode provenance and falls back to persisted `source_time_ns`; PHY Dream uses persisted `source_time_ns`. Dream must never substitute `created_at_ns`/`updated_at_ns` or maintenance timestamps for semantic chronology, and chronology-required duplicate ordering fails closed when source time is unavailable.
90. **PHY canonicalization does not invent provenance evidence.** Direct-authority and supersession rules may promote PHY Memories, but an external `MemorySourceRef` is only provenance identity; owner-local REL corroboration logic does not treat the reference itself as independent source evidence.
...
92. **Community detection is owner-local and Memory-projection-versioned.** One current snapshot partitions only the Memory nodes participating in owner-local Memory-to-Memory Graph history and records the exact `memory_graph_version` from which it was derived; Entity/Observation nodes and cross-owner nodes cannot participate unless a later decision explicitly changes that projection.
93. **Community identity is exact-membership identity, not continuity.** Community identity derives from durable owner UUID plus sorted member `MemoryId`s. Changed membership produces a new ID; continuity is a separate deterministic lineage relation derived from consecutive snapshots.
...
95. **Community names cannot become Graph authority.** Dream-generated and user-authored names are clock-neutral metadata authored against exact `CommunityId`; neither can change membership, Graph authority, traversal, identity, or semantic clocks. User names may resolve through clear lineage; Dream names may resolve only while cumulative Jaccard similarity to their original naming baseline is at least 0.750. Ambiguous lineage inherits nothing.
...
98. **Dream maintenance cadence cannot fabricate semantic chronology.** REL maintenance prefers recoverable Archive/Episode provenance and then persisted `source_time_ns`; PHY uses persisted `source_time_ns`. When semantic chronology is unavailable, last successful Dream time may schedule another pass but cannot become temporal, duplicate-ordering, causal, or supersession evidence. Legacy `updated_at_ns` is only a one-time maintenance bootstrap until a real Dream timestamp exists.
99. **Settled Dream pairs do not consume future candidate capacity.** Candidate discovery excludes active Graph neighbours and unordered Memory pairs already successfully evaluated by Dream before ranking. A completed `none` or withheld evaluation is durable pair history even though it creates no Graph edge; pair-history metadata consumes no semantic version.
100. **Extraction-time temporal status and Dream lifecycle are distinct Memory semantics.** Insomnia persists each retained group's `current|future|historical` proposition-time classification as `Memory.temporal_status`; Dream owns `lifecycle_state` independently. Legacy records with no persisted classification reopen as `unknown` and may be explicitly backfilled, but neither age nor Dream state may be used to invent the lost classifier result.
101. **A non-empty PHY Identity set has exactly one active Identity.** Identity content revision and active selection are separate Ego mutations. Creating the first Identity activates it; creating or editing another Identity does not implicitly switch; an active Identity with alternatives cannot be deleted until another live Identity is selected. Legacy single-Identity records reopen as one active `Default` Identity.
102. **Nondeterministic Chronos results cannot bypass deterministic temporal semantics.** Temporal inference runs only for unresolved indications. Every accepted non-empty canonical answer must deterministically reparse as the same indication kind before persistence. Durable Memory temporal inference is bound to exact `MemoryBodyId + source_time_ns`; a mismatched binding is stale and cannot be carried forward, while deterministic Chronos products remain derived.
103. **Relationship ownership is containment, not participant ownership.** A Relationship is semantically owned by exactly one REL or PHY. Its participants may reference Entities in other owners without transferring, copying, or merging those Entities.
104. **Cross-owner Entity references are explicit.** Persisted Relationship participants use `EntityRef { owner_id, entity_id }`. Direct publication and current Relationship state require a local-owner participant to resolve to a current Entity in the containing owner; retired local IDs are rejected rather than silently redirected. Historical replay may preserve an older revision whose local Entity was current at that point in history, but such a stale reference cannot remain current after reopen. A foreign-owner participant may remain dangling until that owner is mounted.
105. **Relationship visibility follows the Relationship owner.** Visibility of a participant Entity does not grant visibility into another owner's Relationship records. Runtime composition may include only the active PHY Relationship layer plus authorized active REL Relationship layers.
