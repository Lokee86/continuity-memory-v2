...
Public Memory models are `MemoryId`, `MemoryBodyId`, `MemoryRevisionId`, `MemoryRef`, `MemoryDraft`, `Memory`, `MemoryStats`, and `MemoryError`. `MemoryRef { owner_id, memory_id }` is the current minimal owner-qualified cross-file Memory reference.
