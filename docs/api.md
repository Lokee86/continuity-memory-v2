...
Community IDs identify exact owner-scoped membership, not continuity across changed membership. Split/merge lineage, incremental changed-region invalidation, and community-aware traversal are not part of this API yet. Human-readable names are intentionally absent; Warlock may overlay editable display names without changing Reliquary state semantics.
