...
## Overview
The public API exposes `ReliquaryConfig` and model-switchboard types for machine-local runtime routing; `InteractionRuntime` for transport-neutral live session coordination; `WorkspaceMetadata` for one Reliquary's durable workspace identity/type; and `Reliquary` for the existing full storage/runtime model: Archive history, Episodes, Memories, Graph, Insomnia state, embedded files/source attachments, vector backing/publication, compatibility profiles, reconciliation, and hybrid retrieval. `Cva` remains exported as the compatibility/internal type name. It remains development-stage.
## Exact contract
### Local configuration
...
### Container
Public types include `Container`, `ContainerError`, `ChunkRef { offset, len }`, `FormatVersion`, `ContainerIdentity`, `FileKind`, and `ReliquaryScopeKind`. New Reliquary creation uses the typed container identity; legacy 16-byte CVA headers remain distinguishable. `Reliquary::scope_kind()` reports Organization, Project, or Connection and `Reliquary::is_legacy_cva()` reports the legacy physical form. Container still exposes opaque append/read/chunk enumeration, sync, path/format inspection, and `latest_version()` for the existing global semantic clock.
### Workspace metadata
...
`Cva::set_memory_relation(source, target, kind, active, expected_graph_version)` publishes or retracts one oriented Memory-to-Memory relationship. `Cva::set_memory_relations(changes, expected_graph_version)` applies multiple effective `GraphRelationChange` values as one atomic Graph transaction. Both endpoints must already exist as Memories, self-relations and duplicate identities within one batch are rejected, and the caller must supply the exact current `graph_version`. Already-visible states are removed as idempotent no-ops before publication. One non-empty transaction consumes one CVA-global version and one dense Graph-local version regardless of how many relationship changes it contains.
