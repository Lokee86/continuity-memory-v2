...
`refresh_communities_leiden()` materializes a complete owner-local partition of the current Graph. The baseline projects active relationships to unique undirected structural pairs and runs Leiden modularity with the fixed v1 configuration. It does not mutate Graph relationships or consume a semantic/global version. The persisted snapshot records its derived generation and exact source `graph_version`; `CommunityStats.current` becomes false after any later Graph mutation. Refresh against an already-current v1 snapshot is a no-op and appends no bytes. REL and PHY are always partitioned independently, and files without durable owner UUIDs cannot publish community identity.

Community IDs in this baseline identify exact owner-scoped membership, not continuity across changed membership. Split/merge lineage, affected-region scan-and-merge, and community-aware traversal are not part of this API yet. Human-readable names are intentionally absent; Warlock may overlay editable display names without changing Reliquary state semantics.
