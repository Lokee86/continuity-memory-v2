...
## Notes
No compatibility promise has yet been made for Rust method signatures or development persistence formats. The detachable CLI consumes this public surface only. OpenAI-ready embedding and General-model transport, ChatGPT/Codex device-code acquisition, and the finite Insomnia backlog worker are implemented; Codex provider-native transport/token refresh and the shared long-lived runtime remain next.
