...
Project RELs also expose the repository-neutral project-history correlation seam. `ProjectRepositoryRef { kind, repository_id, project_path }` identifies the Lore/Git repository and selected Warlock project subtree; `ProjectRevisionRef` adds one opaque repository revision; and `ProjectFileRef` adds a revision-relative path plus optional 32-byte content hash. `current_rel_semantic_cut()` reports the current `RelSemanticCut { global_version, archive_version, memory_version }`; `correlate_project_revision(revision)` captures that cut and appends an idempotent `ProjectRevisionCorrelation`. `project_revision_correlations()` and `latest_project_revision_correlation()` read that durable history after reopen; `ReliquaryRuntimeHost::latest_project_revision_correlation()` exposes the current association to long-lived hosts without reopening the REL. Correlation is valid only for Project RELs, does not itself advance the semantic global clock, and requires repository kind plus durable `repository_id` to remain stable across the REL's correlation history; `project_path` may change within that same repository.
