...
## Overview
The public API exposes `ReliquaryConfig` and model-switchboard types for machine-local runtime routing; `InteractionRuntime` for transport-neutral live session coordination; `Reliquary` for the existing full non-user scope storage/runtime model; and the separate `Phylactery` type for user-global Identity state. Every newly created REL/PHY has a durable typed owner ID derived from its persisted scope/type plus UUID. Reliquary includes Archive history, Episodes, Memories, Graph, derived community snapshots, Insomnia state, embedded files/source attachments, vector backing/publication, compatibility profiles, reconciliation, and hybrid retrieval. Phylactery currently exposes source-independent Memories, Graph, derived community snapshots, Packed Vectors, Memory Vectors, and Compatibility Profiles. `Cva` remains exported as the Reliquary compatibility/internal type name. The API remains development-stage.
## Exact contract
### Local configuration
...
### Container
Public types include `Container`, `ContainerError`, `ChunkRef { offset, len }`, `FormatVersion`, `ContainerIdentity`, `FileKind`, and `ReliquaryScopeKind`. `FileKind` is `Reliquary` or `Phylactery`. New typed files persist a UUID in the container header. `owner_id()` derives the canonical external ID from the authoritative scope/type plus UUID: `proj-<uuid>`, `org-<uuid>`, `con-<uuid>`, or `phy-<uuid>`; `owner_uuid()` exposes the raw 16 bytes. New Reliquary creation uses Organization/Project/Connection scope; `Phylactery::create` uses Phylactery identity with no Reliquary scope. Invalid kind/scope combinations are rejected. Legacy 16-byte CVA and earlier 24-byte typed headers remain distinguishable for migration and have no durable owner UUID. `Reliquary::scope_kind()` reports Organization, Project, or Connection. Container still exposes opaque append/read/chunk enumeration, sync, path/format inspection, and `latest_version()` for the existing file-global semantic clock.
