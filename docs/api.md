...
`DreamProcessor::process_memory` / `process_memory_with_pair_concurrency` execute the complete same-owner Dream path for a Reliquary Memory. `process_phylactery_memory` / `process_phylactery_memory_with_pair_concurrency` execute the same classifier/verifier pipeline against a Phylactery and publish only to that PHY. Each path performs candidate discovery, bounded pair inference, verification when required, owner-local Graph publication including chronological duplicate-chain maintenance, and owner-local lifecycle reconciliation.
