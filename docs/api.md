...
Current direct `Phylactery::publish_memory` requires a source-independent `MemoryDraft`: all REL-local Episode/node/conversation provenance fields must be `None`. `source_time_ns` may still be present because it is source-derived semantic chronology rather than a source pointer. Cross-file source export/lineage is not encoded by this API yet.

### Memories

Public Memory models are `MemoryId`, `MemoryBodyId`, `MemoryRevisionId`, `MemoryRef`, `MemoryDraft`, `Memory`, `MemoryStats`, and `MemoryError`. `MemoryDraft`/`Memory` include optional `source_time_ns`, a source-derived semantic timestamp that survives source-independent publication and is distinct from `created_at_ns`/`updated_at_ns` bookkeeping. `MemoryRef { owner_id, memory_id }` is the current minimal owner-qualified cross-file Memory reference.

`Cva::publish_memory(id, expected_revision, draft)` publishes one Reliquary Memory revision after validating Archive/Episode provenance. `Phylactery::publish_memory` uses the same revision/body/mutation semantics but validates that REL-local provenance is absent. `source_time_ns`, when present, is persisted as source-independent semantic chronology and participates in mutation/idempotency equality. A new Memory may omit `id`, in which case its stable ID derives from `mutation_id`; replaying the same mutation/draft is idempotent, while changed content or source-time semantics under the same mutation conflicts. Existing Memory IDs require the exact expected current revision.
...
`Cva::publish_dream_pair` and `Phylactery::publish_dream_pair` require the caller's expected current Graph version and reject stale publication. Duplicate publication requires authoritative source chronology; PHY uses persisted `source_time_ns` and never substitutes Memory creation time. `reconcile_dream_lifecycle` exists on both owners. PHY applies source-independent lifecycle rules but deliberately does not claim REL-style independent-source corroboration because its REL-local authority anchors were stripped during routed publication.
