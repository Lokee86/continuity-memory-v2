...
| Conversation metadata / node ancestry / derived leaf inventory / transcript paths / live branch search | [Architecture](architecture.md), [Storage format](storage-format.md), [Rust API](api.md) | `src/archive.rs`, `src/archive_store.rs`, `src/archive_conversation*.rs`, `src/conversation_metadata_*.rs`, `src/archive_model.rs`, `src/cva_conversation.rs`, `src/conversation_search*.rs` | `src/conversation_tests.rs`, `src/conversation_search_tests.rs`, reconciliation tests, Archive/history tests, corpus smoke |
...
| Memories / revision authority / provenance / temporal status / body-bound routing metadata | [Architecture](architecture.md), [Storage format](storage-format.md), [Rust API](api.md), [Architectural invariants](invariants.md), [Perception subsystem plan](perception-subsystem-plan.md), [ADR 0012](decisions/0012-deterministic-episodes-and-insomnia-memory-authority.md), [ADR 0020](decisions/0020-reliquary-and-phylactery-file-kinds.md) | `src/memory_*.rs`, `src/memory_routing_*.rs`, `src/cva_memory_publish.rs`, `src/cva.rs`, `src/phylactery.rs` | `src/memory_tests.rs`, `src/memory_codec_tests.rs`, `src/memory_routing_tests.rs`, `src/phylactery_tests.rs` |
| Graph persistence / relationship topology / traversal | [Architecture](architecture.md), [Storage format](storage-format.md) | `src/graph_*.rs`, `src/cva_graph.rs`, pinned `arcana` crate | `src/graph_tests.rs` + Arcana traversal/storage tests |
| Derived Graph communities / Leiden scan-and-merge / lineage / Dream semantic names | [Architecture](architecture.md), [Storage format](storage-format.md), [Rust API](api.md), [ADR 0025](decisions/0025-owner-local-derived-communities.md), [ADR 0031](decisions/0031-dream-derived-community-semantic-names.md), [ADR 0032](decisions/0032-community-lineage-and-name-continuity.md), [Community benchmark](community-scan-merge-benchmark-2026-08-27.md) | `src/community_*.rs`, `src/dream_community_naming*.rs`, `src/cva_communities.rs`, `src/phylactery_communities.rs` | `src/community_lineage.rs`, `src/community_tests.rs`, `src/community_scan_merge_tests.rs`, `src/dream_community_naming_tests.rs`, ignored `src/community_scan_merge_bench.rs` |
...
| Dream independent verification / verification policy | [Architecture](architecture.md), [Rust API](api.md), [Dream design and validation record](dream-implementation-plan.md) | `src/dream_verifier*.rs`, `src/dream_pair_context.rs` | `src/dream_verifier_tests.rs` |
...
| Echo execution evidence / historical reasoning sidecar | [Architecture](architecture.md), [Storage format](storage-format.md), [ADR 0014](decisions/0014-echo-historical-reasoning-traces.md) | `src/echo_*.rs` | `src/echo_tests.rs`, migration/reconciliation tests |
| Durable REL/PHY owner identity | [Architecture](architecture.md), [Storage format](storage-format.md), [Rust API](api.md) | `src/container.rs`, `src/container_scan.rs`, `src/cva_lifecycle.rs`, `src/phylactery_lifecycle.rs` | `src/reliquary_tests.rs`, `src/phylactery_tests.rs`, `src/container_tests.rs` |
