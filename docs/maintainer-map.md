...
| --- | --- | --- | --- |
| CVA composition / single physical owner | [Architecture](architecture.md), [ADR 0005](decisions/0005-cva-composition-and-packed-vector-objects.md) | `src/cva.rs`, `src/cva_error.rs` | Archive + packed-vector reopen tests |
...
| Reopen reconstruction/indexing/checkpoint acceleration | [Architecture](architecture.md), [Roadmap](roadmap.md), [Current limitations](current-limitations.md) | `src/cva.rs`, `src/container.rs`, `src/archive_rebuild.rs`, `src/packed_vector_rebuild.rs`, Archive indexes | reopen tests + heap/startup profiling |
...
- Packed-vector storage owns immutable matrix identity/shape only; profiles and domain row mappings remain separate.
