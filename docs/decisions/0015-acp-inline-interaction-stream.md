...
Agent Client Protocol (ACP) provides a more useful integration boundary because it carries the live interaction between a client and an agent. The current ACP **Agent Extensions via ACP Proxies** Draft goes further and explicitly proposes components that sit between client and agent, intercept and forward ACP messages, inject context, transform responses, and provide conversation-aware extensions. That shape closely matches Reliquary's live-capture and context-injection requirements.

The architectural opportunity is broader than IDE integration. Reliquary does not require an IDE to benefit from ACP. Any ACP-speaking predecessor — desktop application, terminal/TUI client, plugin host, chat surface, or another agent client — can conceptually place Reliquary inline before the downstream agent.

## Decision

Reliquary will treat **inline ACP-compatible interaction capture** as the preferred design direction for live agent sessions where an ACP path is available.
...
Reliquary is therefore **inline with the observable agent interaction stream**, not inside the provider's private model execution.

### Capture boundary

The capture rule is simple:

> If an interaction event traverses the Reliquary ACP path and is exposed by the protocol/adapter, Reliquary may persist it automatically according to the owning source-history rules.
...
Reliquary cannot capture data that never traverses the observable integration boundary. Provider-private chain of thought, hidden system context, latent/model state, internal orchestration, or other server-side information that the agent/provider does not emit remains unavailable.

This boundary is intentional. Reliquary preserves **observable interaction history**, not hidden provider internals.
...
Normal live capture should not depend on the downstream model remembering to call a Reliquary MCP tool.

The desired flow is:

```text
new user turn enters ACP path
    -> Reliquary observes/persists source event
    -> Reliquary may retrieve relevant prior state
    -> downstream agent receives the turn + permitted context

agent/tool events return
    -> Reliquary observes/persists exposed events
...
- **MCP/API:** explicit memory/tool plane; search, retrieval, `create_memory`, archive queries, diagnostics, and other requested Reliquary operations.

If MCP-over-ACP stabilizes and proves suitable, one ACP connection may eventually carry both the live interaction path and Reliquary-provided MCP capabilities. Reliquary must not depend on that Draft mechanism until its protocol contract is sufficiently stable.

### ACP is not an IDE dependency

Reliquary must not architect this integration around editor-specific assumptions. The relevant abstraction is **client -> Reliquary -> agent**, not **IDE -> Reliquary -> coding agent**.

An IDE can be one ACP client, but so can a dedicated chat client, desktop Reliquary surface, terminal interface, plugin host, or other agent front end.

This keeps the commercial product boundary coherent:

```text
Reliquary runtime = product/core service
...
- Reliquary must not claim access to hidden provider state merely because it is inline with an ACP-compatible client/agent exchange.
...
As of 2026-08-17, core ACP session communication is established, but the official **Agent Extensions via ACP Proxies** and **MCP-over-ACP** designs remain Draft RFDs. Reliquary should therefore separate the product requirement from the transport detail:
...
- Reliquary gains a natural interception point for retrieval/context injection before downstream agent processing.
...
- Provider-private reasoning and state remain outside Reliquary unless explicitly emitted through the observable stream.
- ACP-specific transport churn is isolated behind the integration adapter rather than entering Archive semantics.
- Inline operation increases the trust/security responsibility of the Reliquary runtime because it can observe potentially sensitive interaction traffic.

## Open implementation decisions

- exact ACP proxy/conductor integration if/when the Draft proxy-chain RFD stabilizes;
- exact hosting/proxy shape for external ACP agents inside or alongside the native Reliquary product surface;
...
- failure policy when the inline Reliquary component is unavailable: fail-open, fail-closed, or user-selectable;
...
Rejected. IDEs are one ACP client category, not the architectural boundary Reliquary needs.
...
Rejected. Reliquary can observe only what the client/agent/provider path actually emits through the supported interface.
