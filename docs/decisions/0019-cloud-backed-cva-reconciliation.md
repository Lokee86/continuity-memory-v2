...
Current divergent reconciliation creates a fresh CVA. Immutable Fragment ranges are revalidated and replayed because they remain valid references to merged Archive nodes. The lexical index is process-local/disposable and therefore rebuilds lazily from those merged Fragments and Files after reopen. Compatibility profiles are immutable vector-space contracts and are preserved. Packed vector matrices, Memory-Vector bindings, Archive-Vector bindings, and Vector Generations are deliberately omitted from a real changed repack because they describe a pre-merge population cut. If the candidate contributes no new semantic records at all, the fresh probe output is discarded and replaced with an exact copy of the canonical/left CVA instead. This makes already-absorbed conflicted copies idempotent without adding reconciliation receipts or other persistent synchronization metadata to the CVA. `canonical_change_required` reports that decision; `vector_rebuild_required` is true only when a changed merge actually retired derived vector state. Re-embedding remains an explicit follow-up because reconciliation itself has no embedding endpoint and must not fabricate vector data.
