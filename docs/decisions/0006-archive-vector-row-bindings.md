...
Compatibility profiles and vector generations are related but distinct concerns. A compatibility profile defines the endpoint-independent contract for one usable vector space. Vector Generations, implemented subsequently in ADR 0007, associate one compatibility profile with one Archive-Vector set and coverage/activation metadata. Putting profile data directly into Archive Vectors would merge those responsibilities and make row-to-Archive identity depend on model configuration.
...
An Archive-Vector set contains no compatibility profile, model, metric, normalization rule, quantization semantics, Archive watermark, or active-generation state.
...
Like packed matrices, Archive-Vector sets are immutable backing objects and do not allocate a global version ticket. VectorGeneration publication defines when a compatibility profile plus Archive-Vector set becomes meaningful active retrieval state and owns that semantic ordering.
...
- `CompatibilityProfile` owns vector-space compatibility evidence and policy.
...
- **Store `compatibility_profile_id` in Archive Vectors:** rejected because profile identity belongs above row-to-Archive identity.
