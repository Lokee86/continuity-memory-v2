...
Prompt tuning no longer needs a full Dream population mutation for every iteration. `Cva::dream_memory_context` exposes the production read-only Memory context builder, and `examples/dream_classifier_tune/` uses it to run the 40 explicit regression pairs directly through `DreamClassifier::classify_pair`. The harness performs no retrieval, verification, Graph publication, or lifecycle change, defaults to 12-way classifier concurrency, and accepts `--system-prompt-file` so a candidate can be measured without changing production classifier v1. The boundary gate is now cleared; the next required step is the full 46-Memory population/web audit using the frozen v2f prompt.
