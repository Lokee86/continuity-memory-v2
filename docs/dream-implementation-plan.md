...
Milestone D settles the required Graph publication behavior for primary Dream relations. `Cva::set_memory_relations` publishes one or several edge changes as one Graph semantic transaction. `publish_dream_pair` treats accepted classifier output as an additive positive assertion rather than authoritative replacement state: topical/recurrent use reciprocal edges; factual/causal/supersedes preserve classifier direction; an already-active exact relation is idempotent; and `none` is a strict no-op that cannot retract prior accepted state. Different accepted semantic relation kinds or directions may therefore coexist for the same Memory pair. Milestone E extends that seam for verified duplicates by publishing the chronological predecessor-chain edges while preserving unrelated semantic relations for the pair; only deterministic duplicate-chain reconciliation retracts obsolete `duplicate_of` predecessor links.

## Implementation sequence

### Phase 1 — candidate context and Graph publication seam — implemented

Candidate context, evaluator-facing pair direction semantics, and publication mechanics are implemented: Memory/body identity, relevant metadata/provenance/source chronology, attached active Graph data, canonical MemoryId A/B ordering, atomic Graph relation batches, additive positive-assertion publication, and classifier-direction preservation. No separate durable pair-evaluation store was introduced.
