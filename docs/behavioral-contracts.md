...
| Insomnia deterministic authority policy rejects invalid authority/provenance combinations, unsupported vague/question authority, pure execution receipts, and residual numbered prompt/phase/step progress; useful resulting state is accepted only after progress framing is removed | `insomnia::candidate_policy_tests::*`, `insomnia::candidate_receipt_policy_tests::*` |
| Tracked Insomnia semantic gold data contains exactly 40 unique source-anchored judgments (32 retain / 8 omit) with explicit authority, provenance, metadata, receipt, semantic-target, and forbidden-elaboration fields | `insomnia::gold_tests::insomnia_gold_set_is_well_formed_and_balanced` |
