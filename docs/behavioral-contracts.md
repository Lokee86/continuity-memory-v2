...
| Ego durable state is owner-local and append-only: PHY Identity/Personality round-trip, REL rejects PHY-only persona records, PHY/REL Anchors persist/update/tombstone, cached web synthesis records its source Memory version, source watermarks cannot point beyond current owner Memory state, optimistic revisions reject stale writes, identical writes do not advance Ego state, corrupt future watermarks fail on reopen, and divergent REL reconciliation fails closed rather than dropping unsupported Ego state | `ego_tests::*` |
...
| Community performance/routing experiments are opt-in validation, not skipped correctness coverage; the default test suite contains no ignored tests and the experiment modules compile only with the `community-benchmarks` feature | default `cargo test`; `cargo test --features community-benchmarks --no-run` |
...
| The production disposable lexical index remains exactly equivalent to the brute-force full-fragment scan; the brute-force implementation is compiled only for tests as an equivalence oracle | `lexical_index_tests::indexed_lexical_results_match_full_scan_exactly` |
...
| Insomnia structured extraction requires explicit candidate authority kind (`direct`/`correction`/`adoption`/`retention`) and persists fixed-group temporal classification (`current`/`future`/`historical`) through candidate parsing into authoritative Memory revisions; legacy R2 authority and legacy R2–R5 temporal status reopen as `unknown` rather than inferred semantics, and explicit temporal backfill preserves immutable bodies/provenance/Dream lifecycle | `insomnia::extraction_tests::authority_kind_is_required_and_preserved_by_structured_extraction`, `insomnia::worker_vector_tests::backlog_drain_automatically_vectorizes_created_memories`, `memory_codec_tests::*`, `memory_tests::temporal_status_backfill_preserves_body_and_dream_state`, `phylactery_tests::temporal_status_backfill_preserves_phy_source_reference` |
