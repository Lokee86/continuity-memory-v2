...
| Divergent CVA reconciliation repacks Archive source state, Episodes/Fragments, Memory revisions, durable Insomnia completions, file-to-Memory links, and compatibility profiles into a fresh CVA; grouped completions are re-ticketed, lexical state rebuilds from merged Fragments, stale vector state is retired, and incompatible Branch/Memory/completion revisions fail closed | `cva_reconcile_merge_tests::*`, `cva_reconcile_memory_tests::*`, `cva_reconcile_grouped_tests::reconcile_replays_grouped_insomnia_memory_records`, `cva_reconcile_link_tests::reconcile_replays_file_memory_links_after_targets`, `cva_reconcile_guard_tests::*`, `cva_reconcile_derived_tests::reconcile_repacks_fragments_and_retires_stale_vector_state` |
