...
| Divergent CVA reconciliation replays unrelated Archive source state, preserves attachments, surfaces Branch conflicts, and refuses unsupported divergent Memory/Insomnia completion state | `cva_reconcile_merge_tests::reconcile_merges_unrelated_divergent_nodes`, `cva_reconcile_merge_tests::reconcile_replays_ingested_turn_attachments`, `cva_reconcile_merge_tests::reconcile_surfaces_conflicting_branch_revisions`, `cva_reconcile_guard_tests::reconcile_refuses_divergent_memories_until_memory_replay_exists`, `cva_reconcile_guard_tests::reconcile_refuses_divergent_insomnia_completion` |
