...
| Current typed REL/PHY headers persist a durable UUID and derive canonical owner IDs from scope/type plus UUID | test rewrite pending during the owner-identity format repair; CLI creation smoke verified `proj-<uuid>` output |
| Reliquary comparison requires the same durable owner ID and distinguishes identical, one-side-ahead, and divergent physical histories | reconciliation tests require fixture rewrite from legacy WorkspaceMetadata identity to copied typed RELs |
...
| Dream lifecycle advances successfully completed active `extracted` sources to `knowledge` unless stronger canonical evidence applies, conservatively archives redundant extracted duplicates, projects verified supersession into archival/`superseded_by`, promotes direct/correction authoritative current-state classes, requires distinct user authority anchors for duplicate corroboration, inherits canonical status through a unique verified superseder, and is idempotent | `dream_lifecycle_tests::*`, `dream_canonical_tests::*` |
| The bounded Dream processor composes candidate retrieval → classification → required verification → Graph publication/duplicate handling → lifecycle completion; inference failure leaves the source lifecycle unchanged, and temporal-only candidates flow through the same end-to-end path | `dream_processor_tests::*` |
...
| Routed Insomnia publishes User Memories source-independently to the explicit PHY before committing the REL receipt, records them as owner-qualified `MemoryRef`s, and reuses a prewritten PHY Memory on retry through deterministic mutation identity | `insomnia::ownership_tests::routed_user_memory_uses_owner_qualified_receipt_and_no_rel_provenance`, `insomnia::ownership_tests::routed_retry_reuses_phy_memory_after_wording_drift` |
| Routed finite workers and the long-lived RuntimeHost vectorize User Memories in the attached PHY without creating Project REL Memories, and RuntimeHost returns the attached PHY on shutdown | `insomnia::worker_vector_tests::routed_backlog_drain_vectorizes_user_memory_in_phylactery`, `runtime_host_tests::runtime_host_routes_user_memory_and_vectors_to_attached_phylactery` |
