...
| Owner-local Leiden refresh partitions current Graph nodes into persisted derived communities, is byte-idempotent while Graph is unchanged, becomes stale after Graph mutation, reopens independently in REL/PHY, and requires durable owner identity | `community_tests::*` |
...
| Routed finite workers and the long-lived RuntimeHost vectorize User Memories in the attached PHY without creating Project REL Memories; RuntimeHost then derives independent REL/PHY Dream backlogs only after owner-local vector readiness and returns the attached PHY on shutdown | `insomnia::worker_vector_tests::routed_backlog_drain_vectorizes_user_memory_in_phylactery`, `runtime_host_tests::runtime_host_routes_user_memory_and_vectors_to_attached_phylactery`, `runtime_host_vector_tests::embedding_route_can_be_attached_after_memory_creation` |
