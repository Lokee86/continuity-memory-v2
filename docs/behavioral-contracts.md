...
| Dream publication reconciles primary pair state in one Graph transaction, persists topical/recurrent symmetrically, preserves factual/causal/supersedes direction, honors verifier gating, leaves structural/reference edges alone, and defers duplicate persistence | `dream_publisher_tests::*`, `graph_tests::relation_batch_*` |
