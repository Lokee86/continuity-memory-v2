...
## Overview
Purpose-built local configuration, encrypted credential persistence, model-switchboard routing/auth attachment, direct OpenAI-ready embedding/General transport, provider-native Codex General/Insomnia transport, Archive, deterministic Episodes, authoritative Memory revisions, durable Insomnia queue/lease/retry state, atomic self-cleaning Insomnia completion, structured extraction with one bounded Archive-evidence round, a configurable concurrent whole-backlog Insomnia worker, immutable Memory Vectors, packed matrices, Archive row bindings, compatibility profiles, vector-generation publication, exact semantic retrieval, and the original default lexical/hybrid retrieval policy are implemented. The shared long-lived runtime, Codex OAuth refresh, later Graph/Dream/Ego layers, general physical compaction/vacuum, and other production storage hardening remain incomplete.

## Storage limits
- Chunks are uncompressed and lack container-level checksum/authentication/encryption.
- No general object packing, physical compaction, vacuum, reachability, or reclamation exists; failed vector builders and interrupted Insomnia staging may leave unreachable backing chunks. Insomnia success logically collapses prior operational history immediately, but physical bytes from superseded Work records are reclaimed only once general CVA compaction exists.
...
- Deterministic Archive Episodes, authoritative Memory revision storage, durable Insomnia queue/lease/retry state, and compact successful completion receipts are implemented.
- Insomnia can invoke a General endpoint, validate structured candidates against authoritative Episode user turns, preserve the explicit-retention contract, perform one bounded read-only Archive evidence round, and publish idempotent Memory revisions. Evidence supports exact historical turns, bounded ancestry ranges, and lexical Archive search; it is capped at four requests and 64 turns / 128 KiB, cannot provide user authority, and external assistant provenance must have been returned by that evidence round. Successful Episode processing now publishes zero-or-more new Memory records plus the completion outcome through one atomic completion chunk.
