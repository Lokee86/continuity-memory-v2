...
- Migration into the new 40-byte typed owner-ID header is not implemented yet. Legacy 16-byte CVAs and earlier 24-byte typed REL/PHY files reopen without a durable owner UUID and cannot reconcile until migrated. The Graph owner remains a narrow compatibility exception: pre-Graph CVAs reopen with empty Graph state and write the Graph format marker lazily on the first graph mutation.
