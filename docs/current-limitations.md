...
- CVA cloud-conflict support now includes `Cva::compare` plus fresh semantic repacking for divergent Archive source state, Files, Branches, immutable Episodes/Fragments, Memory revisions, durable Insomnia completion receipts, file-to-Memory links, and compatibility profiles. Mutation/revision/completion conflicts fail closed and failed output is removed. Disposable lexical state is rebuilt lazily from replayed merged Fragments/Files. Packed vectors, Memory/Archive vector bindings, and Vector Generations are intentionally retired from divergent output and `vector_rebuild_required` reports that a verified embedding rebuild is needed. Automatic vector rebuild, provider conflicted-copy discovery, richer user-facing conflict reports, and canonical-file promotion are not yet implemented.
