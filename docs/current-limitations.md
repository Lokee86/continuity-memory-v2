...
## Overview
Purpose-built local configuration, encrypted credential persistence, model-switchboard routing/auth attachment, direct OpenAI-ready embedding/General transport, provider-native Codex General/Insomnia transport, optional singleton CVA workspace metadata (`id`, `name`, `workspace_type`), Archive, per-turn source ingestion, a transport-neutral synchronous `InteractionRuntime` with explicit session resume and streamed completed-message assembly, native source attachments, standalone embedded files, filename-only file search, explicit file-to-Memory links, deterministic Episodes, authoritative Memory revisions, Insomnia processing, immutable Memory Vectors, packed matrices, Archive row bindings, compatibility profiles, vector-generation publication, exact semantic retrieval, and the default lexical/hybrid retrieval policy are implemented. There is still no shared long-lived host loop, general workspace/CVA application surface, Warlock product integration, external live interaction adapter, production provider importer suite, Codex OAuth refresh, later Echo/Graph/Dream/Ego layers, general physical compaction/vacuum, or other production storage hardening.
...
- No Warlock workspace integration or coherent workspace/CVA application surface exists yet. Users cannot currently administer files, Memories, conversations, provenance, selective import/export, or lifecycle state through the intended Warlock product surface.
