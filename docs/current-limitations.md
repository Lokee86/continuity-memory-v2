...
- CVA cloud-conflict support now includes `Cva::compare` plus a first `Cva::reconcile` path. Divergent Archive source Nodes, attachment-bearing turns, standalone Files, and Branch revisions can be replayed into a new validated output CVA; incompatible owner invariants surface as errors and failed output is removed. Fragment/Episode replay/rebuild, divergent Memories, durable Insomnia completion replay, file-to-Memory links, derived vector/index rebuild, provider conflicted-copy discovery, and canonical-file promotion are not yet implemented.
