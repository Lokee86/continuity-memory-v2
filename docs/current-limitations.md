...
- CVA cloud-conflict support now includes `Cva::compare` plus semantic `Cva::reconcile` for divergent Archive source state, Files, Branches, immutable Episodes, Memory revisions, durable Insomnia completion receipts, and file-to-Memory links. Mutation/revision/completion conflicts fail closed and failed output is removed. Fragment rebuild, derived vector/index rebuild or retirement, provider conflicted-copy discovery, richer user-facing conflict reports, and canonical-file promotion are not yet implemented.
