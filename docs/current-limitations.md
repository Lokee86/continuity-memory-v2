...
## Overview
Purpose-built local configuration, encrypted credential persistence, model-switchboard routing/auth attachment, direct OpenAI-ready embedding/General transport, provider-native Codex General/Insomnia/Dream transport, optional singleton CVA workspace metadata (`id`, `name`, `workspace_type`), Archive, derived conversation/leaf inventory and exact-leaf transcript reads, per-turn source ingestion, a transport-neutral synchronous `InteractionRuntime`, durable assistant-stream checkpoints, interruption recovery, completed-message publication, and the in-process `ReliquaryRuntimeHost` are implemented. The host provides long-lived Insomnia workers, inactivity finalization, dynamic General/embedding capability attachment, and continuous missing-Memory-vector backfill while preserving short CVA lock intervals around model/embedding calls. Native source attachments, embedded files, deterministic Episodes, authoritative Memory revisions, Graph storage/topology, immutable Memory Vectors, packed matrices, compatibility profiles, vector generations, lexical/semantic/hybrid retrieval, bounded Dream candidate retrieval, transient Dream pair classification, and independent Dream verification are also implemented. Remaining gaps include the full workspace/CVA management surface, external live interaction adapters, production provider importer coverage, later Dream publication/lifecycle stages, Echo/Ego services, physical compaction/vacuum, and production storage hardening.
...
- Memory Vector storage and automatic missing-body embedding are implemented in both the finite drain and the long-lived host. A Memory is authoritative before embedding. The host probes/embeds outside the interaction lock, commits missing bindings under a short lock, and retries unresolved work on later wakes. Echo, Dream, Ego, production importers, and the native product/management surfaces remain incomplete. Graph persistence, oriented relationship mutation/retraction, reopen reconstruction, adjacency queries, and shortest-path traversal are implemented, but Dream inference, relationship provenance/evidence metadata, lifecycle promotion/canonicalization, and automated reconsideration remain future work.
