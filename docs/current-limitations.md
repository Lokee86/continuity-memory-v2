...
## Overview
Purpose-built local configuration, encrypted credential persistence, model-switchboard routing/auth attachment, direct OpenAI-ready embedding/General transport, provider-native Codex General/Insomnia transport, optional singleton CVA workspace metadata (`id`, `name`, `workspace_type`), Archive, derived conversation/leaf inventory and exact-leaf transcript reads, per-turn source ingestion, a transport-neutral synchronous `InteractionRuntime`, durable assistant-stream checkpoints, interruption recovery, completed-message publication, and the in-process `ReliquaryRuntimeHost` are implemented. The host provides long-lived Insomnia workers, inactivity finalization, dynamic General/embedding capability attachment, and continuous missing-Memory-vector backfill while preserving short CVA lock intervals around model/embedding calls. Native source attachments, embedded files, deterministic Episodes, authoritative Memory revisions, Graph storage/topology, immutable Memory Vectors, packed matrices, compatibility profiles, vector generations, and lexical/semantic/hybrid retrieval are also implemented. Remaining gaps include the full workspace/CVA management surface, external live interaction adapters, production provider importer coverage, later Echo/Dream/Ego services, physical compaction/vacuum, and production storage hardening. The Graph foundation is implemented, but Dream does not yet infer or manage relationships automatically.
