...
## Overview
Purpose-built local configuration, encrypted credential persistence, model-switchboard routing/auth attachment, direct OpenAI-ready embedding/General transport, provider-native Codex General/Insomnia/Dream transport, durable typed REL/PHY owner UUIDs, Archive, derived conversation/leaf inventory and exact-leaf transcript reads, per-turn source ingestion, a transport-neutral synchronous `InteractionRuntime`, durable assistant-stream checkpoints, interruption recovery, completed-message publication, and the in-process `ReliquaryRuntimeHost` are implemented. The host provides long-lived Insomnia workers, inactivity finalization, dynamic General/embedding capability attachment, and continuous missing-Memory-vector backfill while preserving short CVA lock intervals around model/embedding calls. Native source attachments, embedded files, deterministic Episodes, authoritative Memory revisions, Graph storage/topology, immutable Memory Vectors, packed matrices, compatibility profiles, vector generations, lexical/semantic/hybrid retrieval, bounded Dream candidate retrieval, classifier-v2 pair classification, independent verification, atomic Graph publication, chronological duplicate chains, lifecycle projection, deterministic canonical promotion, temporal analysis, and bounded shared-runtime Dream processing are also implemented. Remaining gaps include the full workspace/CVA management surface, external live interaction adapters, production provider importer coverage, Echo/Ego services, Dream relationship provenance/evidence persistence and automated reconsideration, physical compaction/vacuum, and production storage hardening.
...
- Archive, Memories, Graph, and Vector Generations are separate mutable semantic timelines with dense local `u64` watermarks and shared CVA-global ordering. Insomnia queue/lease/retry state is operational and consumes no semantic clock.
...
- `ReliquaryRuntimeHost` now applies the default 15-minute inactivity policy while it is alive. There is no scheduler when Reliquary is used only through one-shot `Cva`/`InteractionRuntime` calls; the embedding application host owns that lifetime.
...
- Memory Vector storage and automatic missing-body embedding are implemented in both the finite drain and the long-lived host. A Memory is authoritative before embedding. The host probes/embeds outside the interaction lock, commits missing bindings under a short lock, and retries unresolved work on later wakes. Echo, Ego, production importers, and the native product/management surfaces remain incomplete. Dream inference, Graph publication, duplicate handling, lifecycle projection, canonical promotion, temporal analysis, and bounded shared-runtime processing are implemented; persistent relationship provenance/evidence metadata and automated reconsideration remain future work.
