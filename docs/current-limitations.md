...
## Overview
Purpose-built local configuration, encrypted credential persistence, model-switchboard routing/auth attachment, direct OpenAI-ready embedding/General transport, provider-native Codex General/Insomnia transport, Archive, per-turn source ingestion, native source attachments, standalone embedded files, filename-only file search, explicit file-to-Memory links, deterministic Episodes, authoritative Memory revisions, Insomnia processing, immutable Memory Vectors, packed matrices, Archive row bindings, compatibility profiles, vector-generation publication, exact semantic retrieval, and the default lexical/hybrid retrieval policy are implemented. There is still no shared long-lived runtime, general CVA management surface, native product UI, live interaction adapter, production provider importer suite, Codex OAuth refresh, later Echo/Graph/Dream/Ego layers, general physical compaction/vacuum, or other production storage hardening.
...
- `Cva::ingest_turn` can synchronously accept one complete source turn with attachments, but no persistent runtime/service/API continuously receives live turns. The development graph importer is currently the only exposed driver of that ingestion path.
- No general CVA management API/service or native product UI exists yet. Users cannot currently administer files, Memories, conversations, provenance, selective import/export, or lifecycle state through a coherent product surface.
- No live ACP-compatible capture/proxy adapter is implemented yet. ACP is now scoped as a first-class external interoperability adapter rather than the primary Continuity user interface; the native product surface and all external adapters are planned to converge through one transport-neutral live runtime. ADR 0015 governs ACP capture semantics and ADR 0016 governs the broader product/runtime boundary.
