...
- CVA cloud-conflict support currently stops at `Cva::compare`: same-workspace identity and physical identical/ahead/diverged classification are implemented, but semantic tail replay, automatic merge, unresolved-conflict reporting, derived-state rebuild, and safe merged-file promotion are not yet implemented.
