...
| `src/embedding_endpoint.rs`, `src/embedding_profile_*` | Endpoint abstraction, profile probing/fingerprinting, durable profile identity, tests | [Architecture](architecture.md), [Rust API](api.md), [ADR 0007](decisions/0007-embedding-profiles-and-vector-generations.md) |
| `src/vector_generation_*` | Generation format, local/global ordering, current-per-profile state, validation, reopen, tests | [Architecture](architecture.md), [Storage format](storage-format.md), [Versioning plan](version-history-plan.md), [ADR 0007](decisions/0007-embedding-profiles-and-vector-generations.md) |
