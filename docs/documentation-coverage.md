...
| `src/model_switchboard*.rs`, `src/model_auth.rs`, `src/configured_general.rs` | General/Insomnia/Dream/Embedding capability routing, provider auth validation, configured General dispatch | [Architecture](architecture.md), [Local configuration](configuration.md), [Rust API](api.md), [ADR 0009](decisions/0009-expandable-model-switchboard.md) |
...
| `src/memory_*.rs`, `src/cva_memory_publish.rs` | Memory bodies, immutable revisions, persisted authority kind, publication/versioning, provenance validation, legacy R2 compatibility | [Architecture](architecture.md), [Storage format](storage-format.md), [Rust API](api.md), [ADR 0012](decisions/0012-deterministic-episodes-and-insomnia-memory-authority.md) |
...
| `src/dream_candidate_*.rs`, `src/dream_candidates.rs` | Read-only bounded Dream Memory candidate discovery, stored-vector semantic lane, source-time prior quota, lexical/metadata fallback, and Graph-context attachment | [Architecture](architecture.md), [Rust API](api.md), [Dream implementation plan](dream-implementation-plan.md) |
| `src/dream_canonical.rs`, `src/dream_lifecycle*.rs` | Deterministic canonical promotion, duplicate corroboration, supersession inheritance, and lifecycle projection | [Architecture](architecture.md), [Rust API](api.md), [Dream implementation plan](dream-implementation-plan.md), [Behavioral contracts](behavioral-contracts.md) |
| `src/dream_classifier*.rs`, `src/dream_pair_context.rs` | Canonical pair ordering, strict Dream relation/direction/evidence classification, transient bounded candidate-set evaluation | [Architecture](architecture.md), [Rust API](api.md), [Dream implementation plan](dream-implementation-plan.md), [Behavioral contracts](behavioral-contracts.md) |
| `src/dream_verifier*.rs` | Independent categorical relation/direction/evidence verification and deterministic accept/reject/uncertain policy | [Architecture](architecture.md), [Rust API](api.md), [Dream implementation plan](dream-implementation-plan.md), [Behavioral contracts](behavioral-contracts.md) |
...
| `src/dream_duplicate.rs`, `src/dream_duplicate_index.rs`, `src/dream_source_time.rs` | Chronological duplicate predecessor-chain publication, authoritative source-time ordering, derived graph-versioned B-tree component index, and atomic chain rewiring | [Architecture](architecture.md), [Rust API](api.md), [Dream implementation plan](dream-implementation-plan.md), [Behavioral contracts](behavioral-contracts.md) |
| `src/dream_lifecycle*.rs`, `src/dream_processor*.rs` | Graph-derived duplicate/supersession lifecycle projection, successful `extracted -> knowledge` completion, and first bounded end-to-end Dream orchestration | [Architecture](architecture.md), [Rust API](api.md), [Dream implementation plan](dream-implementation-plan.md), [Behavioral contracts](behavioral-contracts.md) |
| `src/dream_temporal*.rs` | Derived deterministic temporal parsing, source-time-relative resolution, recurrence identity, temporal candidate matching, and temporal classifier context | [Architecture](architecture.md), [Rust API](api.md), [Dream implementation plan](dream-implementation-plan.md), [Current limitations](current-limitations.md), [Behavioral contracts](behavioral-contracts.md) |
...
| `examples/dream_validate.rs`, `examples/dream_validate/support.rs`, `corpus/dream-validation-v1.json` | Live Dream candidate/classifier/verifier seam validation over synthetic relation cases and selected Insomnia-generated Memory pairs | [Development](development.md), [Dream implementation plan](dream-implementation-plan.md) |
| `examples/dream_tune/*`, `tools/evaluate_dream_gold.py`, `corpus/dream-gold-v1-tuning-v1.json` | Production-path Dream population tuning over disposable durable-CVA copies, per-stage reporting, targeted/retrieval-only runs, reopen verification, and deterministic gold scoring | [Development](development.md), [Dream implementation plan](dream-implementation-plan.md) |
